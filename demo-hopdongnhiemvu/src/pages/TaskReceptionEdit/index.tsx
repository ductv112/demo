...
import {
  ArrowLeftOutlined, EditOutlined, SolutionOutlined,
...
const TaskReceptionEditPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [form] = Form.useForm();
  const [currentStep, setCurrentStep] = useState(0);
  // Pre-check all items since this is editing an existing mission
  const [checked, setChecked] = useState<Record<string, boolean>>(
    Object.fromEntries(checklistItems.map(item => [item.key, true]))
  );
  ...
  if (!mission) {
    return (
      <Card>
        <Text strong style={{ fontSize: 16 }}>Không tìm thấy nhiệm vụ</Text>
  ...
  // Chặn truy cập nếu đã trình duyệt
    ...
    return (
      <Card>
        <Result
          ...
          subTitle={`Nhiệm vụ ${mission.code} đã được phê duyệt, không thể chỉnh sửa.`}
          extra={<Button type="primary" onClick={() => navigate(`/missions/${id}`)}>Xem chi tiết</Button>}
        />
      </Card>
    );
  }
  ...
  // Chỉ cho sửa khi chưa phê duyệt
  const isLocked = !['draft', 'pending_approval'].includes(mission.status);

  const handleSubmit = () => {
    message.success(`Đã lưu và trình duyệt nhiệm vụ ${mission.code} — chờ lãnh đạo phê duyệt`);
  ...
  const renderStep1 = () => (
    <>
      <Card className="db-chart-card" style={{ borderRadius: 14, marginBottom: 16 }} styles={{ body: { padding: 20 } }}>
        ...
        <Row gutter={16}>
          <Col xs={24} md={8}>
            <Form.Item label="Mã nhiệm vụ" name="documentNumber">
              <Input disabled />
            </Form.Item>
          </Col>
          <Col xs={24} md={8}>
        ...
        <Row gutter={16}>
          ...
          <Col xs={24} md={8}>
            <Form.Item label="Số lượng" name="equipmentQuantity">
        ...
        <Col xs={24} lg={10}>
          <Card className="db-chart-card" style={{ borderRadius: 14, height: '100%' }} styles={{ body: { padding: 20 } }}
            ...
            <Descriptions column={1} size="small" labelStyle={{ fontWeight: 600, color: colors.navy, width: 130, fontSize: 12 }} contentStyle={{ fontSize: 13 }}>
              <Descriptions.Item label="Mã nhiệm vụ">{formValues.documentNumber || mission.code}</Descriptions.Item>
    ...
    return (
      <Card className="db-chart-card" style={{ borderRadius: 14 }} styles={{ body: { padding: 20 } }}>
        ...
        <div style={{ background: '#f0f7ff', borderRadius: 10, padding: '14px 18px', marginBottom: 16, borderLeft: `4px solid ${colors.navy}` }}>
          <Text type="secondary" style={{ fontSize: 11 }}>Mã nhiệm vụ</Text>
          <div><Text strong style={{ fontSize: 18, color: colors.navy }}>{mission.code}</Text> <Tag color="processing">Đang chỉnh sửa</Tag></div>
        </div>
        <Descriptions bordered column={{ xxl: 2, xl: 2, lg: 2, md: 1, sm: 1, xs: 1 }} size="small"
          ...
          <Descriptions.Item label="PX xử lý">{dept?.name || '—'}</Descriptions.Item>
  ...
  const renderStep5 = () => (
    <Card className="db-chart-card" style={{ borderRadius: 14 }} styles={{ body: { padding: 20 } }}>
      <Result
        ...
        title="Xác nhận lưu & trình duyệt"
        subTitle={
          <div style={{ maxWidth: 500, margin: '0 auto' }}>
            <p>Nhiệm vụ <strong>{formValues.name || mission.name}</strong> ({mission.code}) sẽ được cập nhật và gửi đến <strong>Ban Giám đốc</strong> để phê duyệt.</p>
            <p style={{ color: '#666' }}>Sau khi trình duyệt, nhiệm vụ sẽ không thể chỉnh sửa cho đến khi được phê duyệt hoặc trả lại.</p>
          </div>
        }
        extra={
          <div style={{ background: '#f0f7ff', borderRadius: 10, padding: '12px 24px', display: 'inline-flex', gap: 16 }}>
            <div><Text type="secondary" style={{ fontSize: 11 }}>Mã NV</Text><div><Text strong style={{ color: colors.navy }}>{mission.code}</Text></div></div>
  ...
  return (
    <div style={{ padding: '0 0 24px' }}>
            ...
            <Col>
              <Space size="middle" align="center">
                ...
                <Space size={8} align="center">
                  ...
                  <div>
                    <div style={{ color: '#fff', fontWeight: 700, fontSize: 17 }}>Chỉnh sửa nhiệm vụ</div>
                    <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12 }}>{mission.code} — {mission.name}</div>
                  </div>
                </Space>
              </Space>
            </Col>
      ...
      {/* ─── Steps (cho phép nhảy bước tự do) ─── */}
      ...
      <Form
        ...
        initialValues={{
          documentNumber: mission.code,
          requestingUnit: mission.requestingUnit,
          name: mission.name,
          equipmentType: mission.equipmentType,
          equipmentQuantity: mission.equipmentQuantity,
          deadline: dayjs(mission.deadline),
          description: mission.description,
          technicalRequirements: mission.technicalRequirements,
          qualityRequirements: mission.qualityRequirements,
          missionType: mission.missionType,
          priority: mission.priority,
          complexity: mission.complexity,
          executionScope: mission.executionScope,
          assignedDepartment: mission.assignedDepartment,
        }}
      >
...
export default TaskReceptionEditPage;
