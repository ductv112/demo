...
import {
  ...
  UploadOutlined, InboxOutlined, ToolOutlined,
...
const ProposalCreatePage: React.FC = () => {
  ...
  // ─── Step 3: Khối lượng ────────────────────────────────────
  const [workVolumes, setWorkVolumes] = useState<WorkVolume[]>([]);
  ...
  const [laborSuggested, setLaborSuggested] = useState(false);
  ...
  const validateStep = useCallback(async (step: number): Promise<boolean> => {
    try {
      if (step === 0) {
        if (!selectedMissionId) { message.warning('Vui lòng chọn nhiệm vụ'); return false; }
  ...
  const handleSubmit = () => {
    message.success('Đã lưu và trình duyệt đề xuất & dự toán thành công');
    navigate('/proposals');
  };
    ...
    return (
      <div>
        <Card className="db-chart-card" style={{ borderRadius: 12 }} styles={{ body: { padding: 20 } }}>
          <Form.Item label="Chọn nhiệm vụ" required>
            <Select showSearch style={{ width: '100%' }} placeholder="Chọn nhiệm vụ cần lập đề xuất"
              value={selectedMissionId || undefined} onChange={v => setSelectedMissionId(v)}
              optionFilterProp="label" size="large"
              options={availableMissions.map(m => ({ value: m.id, label: `${m.code} — ${m.name} (${m.equipmentType})` }))} />
          </Form.Item>
          {mission && (
  ...
  const handleFetchConfig = () => {
    ...
    setTimeout(() => {
      ...
      // Trích xuất mã model thiết bị: các từ gồm toàn chữ hoa/số/gạch ngang (P-18, TTLL, 36D6, S-300PMU...)
      const modelParts = equip
        .split(/\s+/)
        .filter(w => /^[A-Z0-9][A-Z0-9-]*[A-Z0-9]$/i.test(w) && /[A-Z0-9]/i.test(w) && !/^[a-zàáâãèéêìíòóôõùúýăđơư]+$/i.test(w))
        .map(w => w.replace(/-/g, ''))
        .slice(0, 2);
      const modelKey = modelParts.length > 0 ? modelParts.join('-') : 'KT';
      const code = `CFG-${modelKey}-V3.0`;
  ...
  // ═══════════════════════════════════════════════════════════
  // STEP 4: VẬT TƯ - THIẾT BỊ
  // ═══════════════════════════════════════════════════════════
  const handleCheckInventory = () => {
    setInventoryLoading(true);
    setTimeout(() => {
      // Mock: đơn giá chuẩn từ QL TC-KT + tồn kho từ QL Kho
      ...
      <Card style={{ marginBottom: 16, borderRadius: 14, border: 'none', overflow: 'hidden' }} styles={{ body: { padding: 0 } }}>
        <div style={{
          ...
          <Space size="middle" align="center">
            ...
            <div>
              <div style={{ color: '#fff', fontWeight: 700, fontSize: 16 }}>Lập đề xuất & dự toán</div>
              <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12 }}>
                {mission ? `${mission.code} — ${mission.equipmentType}` : 'Chọn nhiệm vụ để bắt đầu'}
              </div>
            </div>
          </Space>
        </div>
