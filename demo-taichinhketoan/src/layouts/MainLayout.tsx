...
import { Layout, Menu, Avatar, Dropdown, Badge, Space, Typography, Button, theme, Tag, Segmented, Divider } from 'antd';
import {
  DashboardOutlined,
  FileTextOutlined,
  DollarOutlined,
...
const MainLayout: React.FC = () => {
  ...
  const { currentUser, switchRole, isFinance, isDepartment, isDirector } = useUser();
    ...
    if (isFinance) {
      return [
        ...
        { type: 'divider', style: { margin: '8px 16px', borderColor: 'rgba(255,255,255,0.06)' } },
        {
          ...
          label: 'Danh mục',
          children: [
            { key: '/categories/sources', icon: <FundProjectionScreenOutlined />, label: 'Nguồn kinh phí' },
            { key: '/categories/cost-types', icon: <AppstoreOutlined />, label: 'Loại chi phí' },
            { key: '/categories/departments', icon: <TeamOutlined />, label: 'Phòng ban' },
          ],
        },
      ];
    }
    ...
    if (isDirector) {
      return [
        ...
        { type: 'divider', style: { margin: '8px 16px', borderColor: 'rgba(255,255,255,0.06)' } },
        {
          ...
          label: 'Phê duyệt',
          children: [
            { key: '/approvals/cost-plans', icon: <FileTextOutlined />, label: 'Phê duyệt KHCP' },
            { key: '/approvals/payments', icon: <DollarOutlined />, label: 'Quyết định giải ngân' },
          ],
        },
        {
          key: '/cost-plans',
  ...
  const roleOptions = (['finance', 'department', 'director'] as UserRole[]).map((role) => ({
    label: (
  ...
  const getSelectedKey = () => {
    ...
    if (path === '/') return ['/'];
    if (path.startsWith('/approvals/cost-plans')) return ['/approvals/cost-plans'];
    if (path.startsWith('/approvals/payments')) return ['/approvals/payments'];
    if (path.startsWith('/cost-plans')) return ['/cost-plans'];
    ...
    if (path.startsWith('/categories/sources')) return ['/categories/sources'];
    if (path.startsWith('/categories/cost-types')) return ['/categories/cost-types'];
    if (path.startsWith('/categories/departments')) return ['/categories/departments'];
    if (path.startsWith('/categories')) return ['/categories/sources'];
    return [path];
  };
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout style={{ marginLeft: collapsed ? 80 : 260, transition: 'margin-left 0.2s' }}>
        ...
        >
          ...
          <Space size={16}>
            <AppSwitcher currentApp="taichinhketoan" />
